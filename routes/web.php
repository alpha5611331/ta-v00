...
/* PUBLIK */
Route::get('/', function () {
    if (auth()->check()) return redirect()->route('upload.index');
    return view('welcome');
})->name('home');
...
Route::post('/register', function (\Illuminate\Http\Request $request) {
    ...
    return redirect()->route('upload.index')->with('success','Akun berhasil dibuat. Selamat datang di VOXORA, '.$user->name.'!');
})->name('register.post');
...
Route::middleware(['auth'])->group(function () {
    Route::get('/dashboard', fn() => redirect()->route('upload.index'))->name('dashboard');

    /* Upload */
    Route::get('/upload',          [UploadController::class, 'index'])->name('upload.index');
    Route::post('/upload',         [UploadController::class, 'store'])->name('upload.store');
    Route::post('/upload/export',  [UploadController::class, 'export'])->name('upload.export');
    Route::post('/upload/braille', [UploadController::class, 'sendToEduBraille'])->name('upload.braille');
